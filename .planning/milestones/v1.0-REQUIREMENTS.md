# Requirements Archive: v1.0 E2E Testing & Embedding MVP
...
- [x] **CONF-01**: Tuwunel supports port 0 (OS-assigned) and exposes the actual bound port (implemented in Phase 4 embed crate)
...
- [x] **E2EE-01**: User can upload device keys and one-time keys to the server under Shadow
- [x] **E2EE-02**: User can claim another user's one-time keys for establishing an Olm session under Shadow
- [x] **E2EE-03**: Two users can exchange encrypted messages in an E2EE room under Shadow
- [x] **E2EE-04**: E2EE key exchange completes deterministically without timing-dependent retry loops
- [x] **E2EE-05**: SAS verification between two devices completes under Shadow simulation

### Test Scenarios — Network Conditions

- [x] **NET-01**: Tests can specify per-link latency in Shadow network topology (e.g., 200ms RTT)
- [x] **NET-02**: Tests can specify packet loss rates in Shadow network topology (e.g., 2% loss)
- [x] **NET-03**: Tests can specify bandwidth limits per host or link (e.g., 1 Mbit upload)
- [x] **NET-04**: Named topology fixtures exist as reusable configs ("slow mobile", "high latency", "lossy link")
- [x] **NET-05**: E2EE messaging succeeds under 200ms latency and 2% packet loss
...
- [ ] **EMBD-10**: In-memory HTTP transport available via extracted axum Router (no TCP socket needed) — DEFERRED to v2
