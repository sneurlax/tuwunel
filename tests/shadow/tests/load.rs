...
fn shadow_load_100_clients() {
	...
	// Use minimal latency, no packet loss for load test.
	// High bandwidth to avoid bottlenecking 100 clients.
	...
	// D-05: 100 client hosts + 1 server host.
	// stop_time 600s per RESEARCH.md: generous for 100
	// sequential registrations under simulated time.
	...
	// D-09: binary pass/fail -- Shadow exit 0 means all
	// processes met expected_final_state.
	assert!(
		result.success(),
	...
	// LOAD-03: Shadow exit 0 already confirms all 100 clients
	// exited successfully (expected_final_state: "exited" for
	// all client hosts).
	eprintln!(
