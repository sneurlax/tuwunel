...
fn shadow_sas_verify() {
	...
	// Build Shadow config for SAS verification 3-host simulation.
	// stop_time "180s": SAS has ~8 protocol round-trip messages,
	// needs generous time for all to-device events to be routed.
	// alice at "5s", bob at "15s": bob needs alice to create the
	// encrypted room and upload keys first. seed 42: deterministic.
	...
	// Assert alice completed the full SAS verification flow
	assert!(
		alice_stderr
	...
	// Assert bob completed the full SAS verification flow
	assert!(
	...
	// Assert key protocol steps were observed
	assert!(
		alice_stderr
